...
class Camera(nn.Module):
    def __init__(self, colmap_id, R, T, FoVx, FoVy, fx, fy, cx, cy, image, image_name, uid,
        ...
        if image is not None:
            ...
            self.original_image = (image / 255.0).clamp(0.0, 1.0).to(self.data_device)
        ...
        if not self.world_view_transform.is_contiguous():
            # print("world view trans is not contiguous")
            self.world_view_transform = self.world_view_transform.contiguous()  # PyTorch 1.12.1需要显式连续
        if self.fx is None:
        ...
        if not self.camera_center.is_contiguous():
            # print("cam center is not contiguous")
            self.camera_center = self.camera_center.contiguous()  # PyTorch 1.12.1需要显式连续
