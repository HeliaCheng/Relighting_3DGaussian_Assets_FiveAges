...
def render_set(model_path, name, iteration, views, gaussians, pipeline, background, pbr_kwargs=None):
    ...
    for idx, view in enumerate(tqdm(views, desc="Rendering progress")):
        results = render_fn(view, gaussians, pipeline, background, scaling_modifier=1.0,dict_params=pbr_kwargs)
        ...
        if gaussians.use_pbr:
            save_image(results["pbr"], os.path.join(pbr_path, '{0:05d}'.format(idx) + ".png"))
...
def render_sets(dataset : ModelParams, pipeline : PipelineParams, skip_train : bool, skip_test : bool):
    with torch.no_grad():
        ...
        scene = SceneFromGaussians(args,gaussians,train_cameras,test_cameras,load_iteration=20000)  # 需要定义一个专门接收已有 Gaussians 的 Scene
