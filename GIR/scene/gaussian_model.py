...
class GaussianModel:
    ...
    def load_ply_safe(self, path):
        ...
        albedo_init = np.zeros((xyz.shape[0], 3), dtype=np.float32)  
        metallic_init = np.zeros((xyz.shape[0], 1), dtype=np.float32)
        roughness_init = np.zeros((xyz.shape[0], 1), dtype=np.float32)
        ...
        self.active_sh_degree = self.max_sh_degree
        self.max_radii2D = torch.zeros((self.get_xyz.shape[0]), device="cuda")
        self.diffuse_occ = torch.ones((self.get_xyz.shape[0], self.diffuse_sample_num), device="cuda")
